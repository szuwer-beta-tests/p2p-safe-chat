...
from utils import generate_invite_code, decode_invite_code
...
class ModernChatApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.btn_show_code = self.create_button("Show Code (Host)", ChatStyle.ACCENT_COLOR)
        self.btn_show_code.clicked.connect(self.start_host)
        
        self.code_input = QLineEdit()
        self.code_input.setPlaceholderText("Paste invite code...")
        ...
        v_layout.addWidget(self.chat_scroll)
    ...
    def create_chat_bubble(self, username, message, is_me=False):
        ...
        # To avoid bubbles stretching too wide:
        msg_lbl.setMaximumWidth(int(self.width() * 0.7))
    ...
    def render_bubble(self, username, message, is_me=False):
        bubble = self.create_chat_bubble(username, message, is_me)
    ...
    def start_host(self):
        if self.server is None and self.client is None:
            ...
            msg_box.setWindowTitle("Your Invite Code")
            msg_box.setText("Share the code shown in the top bar with your friend.")
            msg_box.setInformativeText(code)
    ...
    def connect_to_host(self):
        if self.server is None and self.client is None:
            ...
            if not code:
                QMessageBox.warning(self, "Notice", "Please paste an invite code in the input field next to the Connect button first.")
                return

            ip, port, _ = decode_invite_code(code)
            if ip and port:
                self.client = ChatClient(ip, port)
                self.client.message_received.connect(self.display_received_message)
                self.client.connection_successful.connect(lambda: self.update_status("Connected"))
