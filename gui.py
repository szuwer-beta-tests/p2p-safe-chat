...
class ModernChatApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.btn_show_code = self.create_button("Host Chat", ChatStyle.ACCENT_COLOR, True)
        ...
        self.btn_connect = self.create_button("Connect", ChatStyle.ACCENT_COLOR, True)
        ...
        self.btn_send = self.create_button("Send", ChatStyle.ACCENT_COLOR, True)
    ...
    def show_contact_context_menu(self, pos):
        ...
        if not item: return
        menu = QMenu(self)
        menu.setStyleSheet(f"background-color: {ChatStyle.PANEL_COLOR}; color: {ChatStyle.TEXT_COLOR}; border: 1px solid #333333;")
        rename_action = menu.addAction("Rename Contact")
    ...
    def rename_contact(self, old_name):
        new_name, ok = QInputDialog.getText(self, "Rename", "New name for contact:", text=old_name)
    ...
    def update_contact_code(self, name):
        new_code, ok = QInputDialog.getText(self, "Update Code", "Paste new code:")
        if ok and new_code:
            for contact in self.app_data.get("contacts", []):
    ...
    def delete_contact(self, name):
        reply = QMessageBox.question(self, "Delete Contact", f"Remove '{name}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def start_host(self):
        if self.server is None and self.client is None:
            last_port = self.app_data.get("host_port", 0)
            self.server = ChatServer(port=last_port)
            ...
            self.server.start()
            self.app_data["host_port"] = self.server.port
            save_app_data(self.app_data)
            self.role = 'host'
